...
class PatternChain:
    ...
    def patternMatch(self):
        if len(self.chainList) == 3:
            if (
                len(self.chainList[0].labels) == 0
                and len(self.chainList[1].labels) == 1
                and len(self.chainList[2].labels) == 0
            ):
                # MATCH (n)-[e:person_person]-(m) RETURN n,e,m 查询
                if (
                    self.chainList[0].variable != ""
                    and self.chainList[1].variable != ""
                    and self.chainList[2].variable != ""
                ):
                    self.matchType = 1
            elif (
                len(self.chainList[0].properties) != 0
                and len(self.chainList[1].labels) == 0
                and len(self.chainList[2].labels) == 1
            ):
                if (
                    self.chainList[1].type == "edge"
                    and self.chainList[1].leftArrow == False
                    and self.chainList[1].rightArrow == False
                ):
                    # MATCH (p:plan {name: "面壁计划"})-[e]-(neighbor:person) RETURN neighbor,p,e # 与面壁计划有关的人有哪些？
                    self.matchType = 2
            elif (
                self.chainList[1].type == "edge" and self.chainList[1].leftArrow == True
            ):
                # MATCH (m:movie {title: 'Forrest Gump'})<-[:acted_in]-(a:person) RETURN a, m  # 参演了Forrest Gump电影的演员有哪些？
                self.matchType = 3
            elif (
                len(self.chainList[0].properties) != 0
                and self.chainList[1].rightArrow == True
                and len(self.chainList[2].labels) == 1
            ):
        ...
        if matchType == 1:
            ...
            if returnGen:
                if self.random_numbers.pop() < 5:
                    ...
                    self.desc = (
                        "所有通过"
                        + self.chainList[1].labels[0]
                        + "类型关系连接的节点对"
                        + self.chainList[0].variable
                        + "和"
                        + self.chainList[2].variable
            ...
            self.desc = (
                ...
                + nodeDesc
                + "有关的"
                + self.cypherBase.getSchemaDesc(self.chainList[2].labels[0])
                + "有哪些?"
            )
        elif matchType == 3:
            # MATCH (m:movie {title: 'Forrest Gump'})<-[:acted_in]-(a:person) RETURN a, m  # 参演了Forrest Gump电影的演员有哪些？
            nodeDesc = self.chainList[0].getDesc()
            self.desc = (
                self.cypherBase.getSchemaDesc(self.chainList[1].labels[0])
                + nodeDesc
                + "的"
                + self.cypherBase.getSchemaDesc(self.chainList[2].labels[0])
                + "有哪些?"
...
class ReturnBody:
        ...
        if len(patternChainList) == 1:
            ...
            if len(patternChain.chainList) == len(self.returnItems):
                for index, item in enumerate(self.returnItems):
                    if (
                        len(item) == 2
                        and item[0] == patternChain.chainList[index].variable
                        and item[1] == 0
                    ):
                        continue
                    else:
            ...
            if matchSuccess:
                ...
                if rand < 0.3:
                    ...
                    self.returnDesc = self.returnDesc + self.cypherBase.mergeDesc(
                        mergeList
                    )
    ...
    def getDesc(self, patternChainList):
        ...
        if self.patternMatch(patternChainList) == False:
            ...
            self.returnDesc = "返回"
            for item in self.returnItems:
                typeDesc = patternChainList[0].getVariableTypeDesc(
                    item[0]
                )  # 待完善，暂时只支持一个patternchain
                if len(item) == 3 and item[1] != 0:
                    if self.random_numbers.pop() < 5:
                        returnDescList.append(
                            item[0]
                            + typeDesc
                            + "的"
                            + item[1]
        ...
        if len(self.orderBy) == 1 and self.DISTINCT == False:
            if self.random_numbers.pop() < 5:
                self.orderDesc = (
                    ...
                    + self.orderBy[0][1]
                    + "属性"
                    + self.cypherBase.getTokenDesc(self.orderBy[0][2])
                    + "排序"
                )
            else:
                self.orderDesc = (
            ...
            self.orderDesc = (
                ...
                + self.orderBy[1][1]
                + "的值"
                + self.cypherBase.getTokenDesc(self.orderBy[1][2])
                + "排列"
            )
        else:
            pass  # 多条sort的情况
