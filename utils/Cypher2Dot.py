...
class Cypher2Dot(object):
    def __init__(cls):
        cls.node_cnt=0             # 标记当前节点的编号
        cls.root_list=[]           # 记录根节点的编号
        cls.dot = Digraph()        # 绘图
    ...
    def Cypher2Dot(cls, t:Tree,ruleNames:list=None, recog:Parser=None):
        ...
        s = escapeWhitespace(cls.getNodeText(t, ruleNames), False) #返回叶子节点的值或者节点类型
        if t.getChildCount()==0: # 叶子节点
            if(s!=' '):
        ...
        for i in range(0, t.getChildCount()):
            cls.node_cnt+=1
            cls.dot.edge(str(cls.root_list[-1]), str(cls.node_cnt)) # graphviz可以支持先定义边，后定义节点。
