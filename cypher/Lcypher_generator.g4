/*
 * Copyright (c) 2015-2019 "Neo Technology," Network Engine for Objects in Lund AB
 ...
 * 
 * This work was created by the collective efforts of the openCypher community. Without limiting the
 * terms of Section 6, any Derivative Work that is not approved by the public consensus process of
 * the openCypher Implementers Group should not be described as “Cypher” (and Cypher® is a
 ...
 * "implementation extensions to Cypher" or as "proposed changes to Cypher that are not yet approved
 * by the openCypher community".
 */
/*
 * @description Cypher for TuGraph (derived from Cypher.g4 in openCypher@b724b18) @created WangTao
 * @date 2019.08.27
 */
grammar Lcypher_generator;
...
EXPLAIN: ('E' | 'e') ('X' | 'x') ('P' | 'p') ('L' | 'l') (
		'A'
		| 'a'
	) ('I' | 'i') ('N' | 'n');

PROFILE: ('P' | 'p') ('R' | 'r') ('O' | 'o') ('F' | 'f') (
		'I'
		| 'i'
	) ('L' | 'l') ('E' | 'e');
...
oC_SinglePartQuery: (( oC_ReadingClause SP?)* oC_Return)
	| (
		(oC_ReadingClause SP?)* oC_UpdatingClause (
			SP? oC_UpdatingClause
		)* (SP? oC_Return)?
	);
...
oC_Match: (OPTIONAL_ SP)? MATCH SP? oC_Pattern (SP? oC_Hint)* (
		SP? oC_Where
	)?;

OPTIONAL_: ('O' | 'o') ('P' | 'p') ('T' | 't') ('I' | 'i') (
		'O'
		| 'o'
	) ('N' | 'n') ('A' | 'a') ('L' | 'l');

MATCH: ('M' | 'm') ('A' | 'a') ('T' | 't') ('C' | 'c') (
		'H'
		| 'h'
	);
...
oC_Delete: (DETACH SP)? DELETE_ SP? oC_Expression (
		SP? ',' SP? oC_Expression
	)*;

DETACH: ('D' | 'd') ('E' | 'e') ('T' | 't') ('A' | 'a') (
		'C'
		| 'c'
	) ('H' | 'h');

DELETE_: ('D' | 'd') ('E' | 'e') ('L' | 'l') ('E' | 'e') (
		'T'
		| 't'
	) ('E' | 'e');
...
oC_InQueryCall:
	CALL SP oC_ExplicitProcedureInvocation (
		SP? YIELD SP oC_YieldItems
	)?;

CALL: ( 'C' | 'c') ( 'A' | 'a') ( 'L' | 'l') ( 'L' | 'l');

YIELD: ('Y' | 'y') ('I' | 'i') ('E' | 'e') ('L' | 'l') (
		'D'
		| 'd'
	);
...
RETURN: ('R' | 'r') ('E' | 'e') ('T' | 't') ('U' | 'u') (
		'R'
		| 'r'
	) ('N' | 'n');

oC_ReturnBody:
	oC_ReturnItems (SP oC_Order)? (SP oC_Skip)? (SP oC_Limit)?;
...
oC_Hint:
	USING SP JOIN SP ON SP oC_Variable
	| USING SP START SP ON SP oC_Variable;

USING: ('U' | 'u') ('S' | 's') ('I' | 'i') ('N' | 'n') (
		'G'
		| 'g'
	);
...
oC_NodePattern:
	'(' SP? (oC_Variable SP?)? (oC_NodeLabels SP?)? (
		oC_Properties SP?
	)? ')';
...
oC_RelationshipDetail:
	'[' SP? (oC_Variable SP?)? (oC_RelationshipTypes SP?)? oC_RangeLiteral? (
		oC_Properties SP?
	)? ']';
...
oC_PowerOfExpression:
	oC_UnaryAddOrSubtractExpression (
		SP? '^' SP? oC_UnaryAddOrSubtractExpression
	)*;
...
oC_BooleanLiteral: TRUE_ | FALSE_;

TRUE_: ( 'T' | 't') ( 'R' | 'r') ( 'U' | 'u') ( 'E' | 'e');

FALSE_: ('F' | 'f') ('A' | 'a') ('L' | 'l') ('S' | 's') (
		'E'
		| 'e'
	);
...
EXISTS: ('E' | 'e') ('X' | 'x') ('I' | 'i') ('S' | 's') (
		'T'
		| 't'
	) ('S' | 's');

oC_ExplicitProcedureInvocation:
	oC_ProcedureName SP? '(' SP? (
		oC_Expression SP? (',' SP? oC_Expression SP?)*
	)? ')';
...
oC_PatternComprehension:
	'[' SP? (oC_Variable SP? '=' SP?)? oC_RelationshipsPattern SP? (
		WHERE SP? oC_Expression SP?
	)? '|' SP? oC_Expression SP? ']';

oC_PropertyLookup: '.' SP? ( oC_PropertyKeyName);

oC_CaseExpression: (
		( CASE ( SP? oC_CaseAlternatives)+)
		| (CASE SP? oC_Expression ( SP? oC_CaseAlternatives)+)
	) (SP? ELSE SP? oC_Expression)? SP? END;
