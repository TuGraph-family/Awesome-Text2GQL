...
def main():
    try:
        """
        Initializes the CorpusGenerator and generates a diverse question-answer corpus based on the given schema file.
        - graph (str): The name of the graph database to use (e.g., "example").
        - schema_file (Path): The path to the schema file (e.g., "examples/generated_schemas/example.json").
        - output_path (Path): The path where the generated corpus will be saved (e.g., "examples/generated_corpus/example_corpus.json").
        - tu_client (TuGraphClient): The graph database client used for querying and schema exploration.
        - llm_client (LlmClient): The language model client used for question generation and translation (e.g., "qwen3-coder-plus-2025-07-22").
        - num_questions_to_generate (int): The number of diverse questions to generate in the initial phase. This controls the variety of questions created from the schema.
        - target_corpus_size (int): The desired total number of question-answer pairs in the final corpus after iterative augmentation.
        """
        graph = "example"
        schema_file = Path(f"examples/generated_schemas/{graph}.json")
        output_path = Path(f"examples/generated_corpus/{graph}_corpus.json")
        tu_client = TuGraphClient(start_host_port="localhost:7070", 
                                  username="admin", 
        ...
        logger.info(f"Phase 2 complete. Saving {len(generator.corpus_res)} seed pairs to {output_path}...")
