...
QUERY_ARCHETYPES = [
    "Aggregation and Counting: Statistics on certain types of nodes or relationships in the graph, such as calculating quantity, sum, average, maximum/minimum values. Example: 'Count the number of all type A nodes in the database.'", 
    "Filtering and Sorting: Filter nodes that meet conditions based on one or more attribute values, and sort the results. Example: 'Find type A nodes where attribute X is greater than [some value] and attribute Y is [some string], sorted by attribute X in descending order.'",
    "Relationship Reachability Query: Query which other nodes can be reached from a specific node through specified relationships. Example: 'Which type B nodes have [R-type relationship] with the type A node named [instance name]?'",
    "Multi-hop Path Query: Query complex paths spanning two or more relationships. Example: 'Which type A nodes can connect to the type C node named [instance name] through type B nodes? (A->B->C)'",
    "Common Neighbors and Association Analysis: Find whether two or more nodes are connected through the same intermediate node, often used to discover indirect connections. Example: 'Which type A nodes and another type A node named [instance name] are both connected to the same type B node? (A1->B<-A2)'",
    "Existence and Boolean Checks: Check whether nodes or patterns that meet specific conditions exist in the graph, typically returning yes or no. Example: 'Does the database contain a type A node whose attribute X value is [some specific value]?'",
    "Attribute Comparison Query: Filter other nodes based on comparisons between different nodes or based on a node's attributes. Example: 'Find all other type A nodes whose attribute X value is greater than that of the type A node named [instance name].'",
    "Path Analysis and Traversal: Focus on analysis of paths themselves, such as finding the shortest path or all possible paths. Example: 'Find the shortest path between the type A node named [instance A] and the type B node named [instance B].'"
]
