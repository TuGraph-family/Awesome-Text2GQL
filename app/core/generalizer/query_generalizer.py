...
class QueryGeneralizer:
    ...
    def generalize(self, query_pattern: List[Clause]) -> List[str]:
        ...
        for path_pattern in path_pattern_list:
            ...
            # add where clause
            compare_expression: CompareExpression = self.schema_graph.match_where_expression(path_pattern.node_pattern_list[0].symbolic_name, path_pattern.node_pattern_list[0].label)
            where_clause: WhereClause = WhereClause(compare_expression=compare_expression)

            # add return clause
            return_body: ReturnBody = self.schema_graph.match_return_body([(path_pattern.node_pattern_list[-1].symbolic_name, path_pattern.node_pattern_list[-1].label)])
